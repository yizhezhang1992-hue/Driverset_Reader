...
videoName = './Invalid_video.mp4'
dataFolderName = './Invalid_folder'
...
class figureToPlot(QtWidgets.QVBoxLayout):
   ...
   def __init__(self,parent=None):
      ...
      self.fileList.addItem("Vehicle_Data")
      self.fileList.addItem("Other_Data")
   ...
   def loadData(self):
      try:
         self.data = np.genfromtxt(self.dataFileName, dtype=float, delimiter=' ')
   ...
   def setColumnInfo(self):   #def info shown depending on index and file selected
      ...
      if (f == 'Vehicle_Data'):
         if (i == 1):
            self.columnInfo.setText('SOC')
         elif (i == 2):
            self.columnInfo.setText('Vehicle_Speed (km/h)')
         elif (i == 3):
            self.columnInfo.setText('Battery_Power (kW)')
         elif (i == 4):
            self.columnInfo.setText('Distance_since_start (km)')
         elif (i == 5):
            self.columnInfo.setText('Slope_percent (%)')
         elif (i == 6):
            self.columnInfo.setText('Consumed_Energy (kWh)')
         elif (i == 7):
            self.columnInfo.setText('Regenerated_Energy (kWh)')
         elif (i == 8):
            self.columnInfo.setText('Charged_Energy (kWh)')
         elif (i == 9):
            self.columnInfo.setText('Profit (RMB)')
   ...
   def plot(self):
      ...
      self.ax.clear()  # or self.ax.cla()
      self.ax.tick_params(axis='both', which='both', labelsize=6)  # 改成你想要的字号
...
class WindowPlot(QtWidgets.QMainWindow):
   ...
   def __init__(self, parent=None):
      ...
      if hasattr(sys, '_MEIPASS'):  #differ between packed app and developed
         self.labelLogoRobesafe.setPixmap(QtGui.QPixmap(os.path.join(sys._MEIPASS, './icons/ZF.jpg')).scaled(self.labelLogoRobesafe.size(), QtCore.Qt.KeepAspectRatio))
      else:
         self.labelLogoRobesafe.setPixmap(QtGui.QPixmap('./icons/ZF.jpg').scaled(self.labelLogoRobesafe.size(), QtCore.Qt.KeepAspectRatio))
      self.labelDrivesafe = QtWidgets.QLabel()
      self.labelDrivesafe.setFixedHeight(100)
      if hasattr(sys, '_MEIPASS'):
         self.labelDrivesafe.setPixmap(QtGui.QPixmap(os.path.join(sys._MEIPASS , './icons/DRP.png')).scaled(self.labelDrivesafe.size(), QtCore.Qt.KeepAspectRatio))
      else:
         self.labelDrivesafe.setPixmap(QtGui.QPixmap('./icons/DRP.png').scaled(self.labelDrivesafe.size(), QtCore.Qt.KeepAspectRatio))
      self.labelLogoUah = QtWidgets.QLabel()   
      self.labelLogoUah.setFixedHeight(100)
      if hasattr(sys, '_MEIPASS'):
         self.labelLogoUah.setPixmap(QtGui.QPixmap(os.path.join(sys._MEIPASS , './icons/SCALAR.jpg')).scaled(self.labelLogoUah.size(), QtCore.Qt.KeepAspectRatio))
      else:
         self.labelLogoUah.setPixmap(QtGui.QPixmap('./icons/SCALAR.jpg').scaled(self.labelLogoUah.size(), QtCore.Qt.KeepAspectRatio))
      ...
      labelTimeWindow = QtWidgets.QLabel('Select Plot Interval (10 mins): ')
      ...
      self.showMessage('Welcome to Trip Visualizer')

      window = QtWidgets.QWidget()
      window.setLayout(vboxGlobal)
      self.setCentralWidget(window)

      self.setWindowTitle('Trip Visualizer')
      self.move(50,100)
      self.show()
   ...
   def startCaptureThread(self):
      ...
      if (videoCorrect and dataCorrect):
         if (self.paused):
            self.paused = False
            print ("pressed start")
            self.showMessage('Sequence started1')
            ...
            if (self.capturing == False):
               ...
               print ("pressed start")
               print(self.video_thread)
               self.showMessage("Sequence started2")
               self.paused = False
               self.capturing = True
               self.start_button.setText('Pause')
               ...
               print ("pressed Pause")
               self.showMessage("Sequence paused3")
   ...
   def startCapture(self):
      ...
      while(self.capturing):
         ...
         if (self.frameNumberChanged):
            newFrameNumber = int(self.sliderVideoFrame.value()*fps)
            num = newFrameNumber
         ...
         if (num >= self.videoReader.get_length()+1):
         ...
         imageQ = QtGui.QImage(frame.tobytes(), frame.shape[1], frame.shape[0], QtGui.QImage.Format_RGB888)

         if (frame.shape[1] != videowidthShow or frame.shape[0] != videoheightShow):
            imageQ = imageQ.scaled(videowidthShow, videoheightShow, QtCore.Qt.KeepAspectRatio,
                       QtCore.Qt.SmoothTransformation)  #resize image to fit
   ...
   def loadScoresData(self):
      scoresFileName = dataFolderName + '/' + 'Scores.txt'
      global dataCorrect
      try:
         self.scoresData = np.genfromtxt(scoresFileName, dtype=float, delimiter=' ')
   ...
   def loadScoreWidgets(self):
      ...
      # self.gridScores.addWidget(verticalLine, 0, 9, 3, 1)
      ...
      self.listScores0[0].setText('Harsh Acc.')
      self.listScores0[1].setText('Harsh Dec.')
      self.listScores0[2].setText('Speed Fluct.')
      self.listScores0[3].setText('Regen. Util.')
      self.listScores0[4].setText('Cruise Ratio')
      self.listScores0[5].setText('Overspeed')
      self.listScores0[6].setText('Weaving')
      self.listScores0[7].setText('Overall')
      self.listScores0[8].setText('Normal')
      self.listScores0[9].setText('Eco')
      self.listScores0[10].setText('Sport')
   ...
   def setInitialStyleSheet(self, label):
      ...
      label.setStyleSheet("""
           ...
           [color="blue"] {background-color: blue; color: black; border-style: outset; border-width: 2px; border-radius: 10px; border-color: beige; font: bold 14px; qproperty-alignment: AlignCenter;}
            """)
   ...
   def updateScoreLabels(self):
      ...
      for n in range(0, 11):
         if (dataCorrect):
            scoreval = self.getScoreForLineCol(i, n+1)
      ...
      for n in range(0, 11):
         if (dataCorrect):
            scoreval = self.getScoreForLineCol(i, n+12)
   ...
   def loadVideo(self):
      ...
      if (videoCorrect):
         ...
         self.sliderVideoFrame.setRange(0, int(self.totalFrames/fps))
